...
phase = time MOD P
foldPlot = plot(phase,norFlux,LINESTYLE=6,XRANGE=[0,P],YRANGE=[0.985,1.002], $
...
earthRadius = 6367              ; km
;; Need to update this per object:
Rs = 1.21 * solarRadius             ; Accepted radius of parent star, in km
Rpkm = sqrt(deltaF * Rs^2)           ; In km
Rp = Rpkm / earthRadius
print,'Exoplanet Radius: ',Rp,' Earth radii'

;; Stellar mass, also need to update this per object:
M = 1.29                       ; Solar masses
;; Semi-Major Axis:
a = ((P / 365.25)^2 * M)^(1./3.)
